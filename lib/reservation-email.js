...
function formatJourney(journey) {
  if (!journey) return "Sin datos de recorrido";
  const pages = Array.isArray(journey.pages) ? journey.pages.map((page) => page.path).join(" → ") : "";
  const selectedItems = Array.isArray(journey.selected_items) ? journey.selected_items.join(" + ") : "";
  return [
    `Canal: ${journey.channel || "sin identificar"}`,
    `Campaña: ${journey.acquisition_campaign || "sin campaña"}`,
    `Página de entrada: ${journey.landing_page || "sin dato"}`,
    `Origen anterior: ${journey.referrer_host || "directo"}`,
    `Recorrido: ${pages || "sin dato"}`,
    `Selección: ${selectedItems || "sin dato"}`,
    `Tiempo hasta reservar: ${journey.duration_seconds == null ? "sin dato" : `${journey.duration_seconds} segundos`}`
  ].join("\n");
}
...
function buildReservationEmail(payload, reservationId) {
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:680px;margin:0 auto;color:#0f172a">
      ...
      <h2>Evento</h2>
      <p>
        <strong>Tipo de evento:</strong> ${escapeHtml(payload.event_type || "por definir")}<br>
        <strong>Niños invitados:</strong> ${escapeHtml(payload.children_count === "" || payload.children_count == null ? "por definir" : payload.children_count)}<br>
        <strong>Region:</strong> ${escapeHtml(payload.event_region || "por definir")}<br>
        <strong>Comuna:</strong> ${escapeHtml(payload.event_commune)}<br>
        <strong>Dirección:</strong> ${escapeHtml(payload.event_address)}<br>
      ...
      <p style="white-space:pre-line">${escapeHtml(formatJourney(payload.journey))}</p>
      <hr>
  ...
  const text = [
    ...
    `Tipo de evento: ${payload.event_type || "por definir"}`,
    `Ninos invitados: ${payload.children_count === "" || payload.children_count == null ? "por definir" : payload.children_count}`,
    `Region: ${payload.event_region || "por definir"}`,
    `Comuna: ${payload.event_commune || ""}`,
    `Direccion: ${payload.event_address || ""}`,
